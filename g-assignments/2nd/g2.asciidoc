...
////
=== Structure of the MIPS Pipeline

Each stage of the pipeline takes inputs from the previous stage of the pipeline
(or the instruction memory) and produces outputs for the next stage of the
pipeline (or writes the result to a register). Non-programmer accessible
__pipeline registers__ are used between the stages to save these intermediate
results. 
////
...
////
(Hint: How many pipeline stages would an instruction pass through in a clock
cycle if we did it the other way around?)
////
...
----
~$ mkdir ~/ark/2nd
~$ cp -r ~/ark/1st/* ~/ark/2nd/
----

Download and unpack the handout archive (`g2-handout-v1.0.tar.gz`) from
Absalon. Replace the files in `~/ark/2nd/asm` with the `asm/*` files from the
handout, and overwrite `~/ark/2nd/mips32.h` with the new version of `mips32.h`.
Your old assembly files, are likely _not_ to work with this assignment, before
you are completely done with this assignment.
...
Or, what is that value in binary?
...
`sw-lw.S`. Print the values of the fields in the IF/ID and ID/EX pipeline
registers at the end of every clock cycle. See `show_status` from
https://github.com/onlineta/ark15/blob/master/g-assignments/1st/g1.asciidoc[G1]
...
The first data hazard is that the data we need for the EX stage in the
...
register is equal to either `id_ex.rs` or `id_ex.rt`, then forward ALU result
to the appropriate field(s) in ID/EX.
...
should be forwarded to `id_ex.jump_target` instead `id_ex.rs_value`.
...
Test that your simulator works with the good old `universe.c`.
